...
if(!class_exists('Fallsky_Customize_Footer')){
	class Fallsky_Customize_Footer extends Fallsky_Customize_Base {
		...
		private function get_instagram_html($username, $number){
			...
			if($fallsky_is_preview){
				...
				if(!empty($feeds) && is_array($feeds)){
					$list_wrap 	= '<ul%s>%s</ul>';
					$item_wrap 	= '<li%s%s><a href="%s"><div class="feed-bg" style="background-image: url(%s);"></a></li>'; 
					$padding 	= absint(fallsky_get_theme_mod('fallsky_site_footer_instagram_space'));
					...
					}, array('template' => $item_wrap, 'number' => $number, 'padding' => $li_padding));

					return sprintf($list_wrap, $ul_margin, implode('', $feeds));
				}
			}
			else{
				return apply_filters('loftocean_instagram_html', '', $username, $number);
			}
		}
		public function instagram_widget(){
			...
			if(class_exists('LoftOcean_Widget_Instagram')
			 && ($fallsky_is_preview || $show_instagram)
			 && !empty($username)){
				...
				if(!empty($feed_html)){
					...
					printf(
						'<div id="fallsky-site-footer-instagram" class="widget fallsky-widget_instagram column-%s%s%s"%s>%s%s</div>',
						$columns,
						...
						!empty($title_text) || $fallsky_is_preview ? sprintf(
							'<h5 class="widget-title%s%s"><a href="%s" target="_blank">%s</a></h5>', 
							$title_class,
							$title_hide, 
							esc_url( sprintf( 'https://www.instagram.com/%s', $username ) ), 
							esc_html( $title_text )
						) : '',
						fallsky_sanitize_html($feed_html)
					);
				}
			}
		}
